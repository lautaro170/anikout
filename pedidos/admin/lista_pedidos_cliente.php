    ...
    <div class="box-body">
      <div class="table-responsive">
       <table class="table table-striped" id="table-pedido">
        ...
        /*  $cdquery="SELECT p.pedidoid,u.Nombre, u.Apellido, u.Preferencias as preferencias, p.fecha,p.armado, COUNT(pp.pedidoid) as cant, u.UsuarioId, p.nota, u.PrecioDelivery as delivery
          FROM pedido p 
          inner join usuario u on p.usuarioid = u.UsuarioId
          inner join productopedido pp on pp.pedidoid = p.pedidoid
          group by p.pedidoid order by u.Nombre;";
*/

          $cdquery="SELECT p.pedidoid,u.Nombre, u.Apellido, u.Preferencias as preferencias, p.fecha, p.fecha_entrega, p.armado, COUNT(pp.pedidoid) as cant, u.UsuarioId, p.nota,  gze.precio as delivery, u.PrecioDelivery as deliveryIndividual
          FROM pedido p 
          INNER JOIN usuario u ON p.usuarioid = u.UsuarioId
          LEFT JOIN zonas_envio ze ON u.ZonaEnvio = ze.ZonaId
          LEFT JOIN grupo_zonas_envio gze ON ze.grupo_zona_envio_id = gze.id
          INNER JOIN productopedido pp ON pp.pedidoid = p.pedidoid
          WHERE p.fecha BETWEEN '" . getPedidoStartDate($today) . "' AND  '" . date("Y-m-d")."'
          group by p.pedidoid, p.fecha_entrega ORDER BY p.fecha_entrega DESC, u.Nombre;";
          ...
          while ($cdrow=mysqli_fetch_array($cdresult)) {
            ...
            $nombrePedido = getPedidoNombre($cdrow["fecha"], $pid);
              ...
              $user = $cdrow["Nombre"] . " " . $cdrow["Apellido"];
              $delivery = ($cdrow["delivery"] != null) ? $cdrow["delivery"] : $cdrow["deliveryIndividual"];
                $oldUser = $uid;
                ?>
                <tr data-target="#pedido<?php echo $pid; ?>" data-ui="<?php echo $uid; ?>" class="keydiv <?php echo $classArmado; ?>">
                  <td class="nombre-cliente">
                    <?php
                    echo $user . " - Delivery: " . $delivery;
                    ?>
                  </td>
